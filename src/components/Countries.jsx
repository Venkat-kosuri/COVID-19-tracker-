...
const Countries = () => {
  ...
  const handleContinentToggle = (continent) => {
    if (expandedContinents.includes(continent)) {
      setExpandedContinents(
        expandedContinents.filter((item) => item !== continent)
      );
    } else {
      setExpandedContinents([...expandedContinents, continent]);
    }
  };
  ...
  return (
    ...
    {searchedCountry ? (
      <div className="mb-4 p-4 border border-gray-300 rounded">
        ...
        <p>Population: {data?.response?.[0]?.population}</p>
        <p>Total Covid Cases: {data?.response?.[0]?.cases?.total}</p>
      </div>
    ) : (
